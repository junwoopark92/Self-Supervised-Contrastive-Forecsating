...
class Model(nn.Module):
    ...
    def __init__(self, configs, conv_kernel=[12, 16]):
        ...
        # embedding
        self.dec_embedding = DataEmbedding(configs.enc_in, configs.d_model, configs.embed, configs.freq, configs.wo_freq,
                                           configs.dropout)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        mark_init_dec = torch.cat([x_mark_enc[:, -self.seq_len:, :], x_mark_dec[:, -self.pred_len:, :]], dim=1)
        dec_out = self.dec_embedding(seasonal_init_dec, mark_init_dec)
